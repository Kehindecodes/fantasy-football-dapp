...
async function main() {
	...
	const registerTx = await userContract.registerUser('Akure');
	await registerTx.wait();

	const akure = await userContract.getUser(accountAddress);
	console.log('Registered user:', akure);
	...
	const updatePointsTx = await userContract.updatePoints(120);
