...
async function main() {
	...
	const registerTx = await userContract.registerUser('Tolu');
	await registerTx.wait();

	const tolu = await userContract.getUser(accountAddress);
	console.log('Registered user:', tolu);
	...
	const updatePointsTx = await userContract.updatePoints(100);
