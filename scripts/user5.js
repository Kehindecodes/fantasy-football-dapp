...
async function main() {
	...
	const registerTx = await userContract.registerUser('Sodiq');
	await registerTx.wait();

	const sodiq = await userContract.getUser(accountAddress);
	console.log('Registered user:', sodiq);
	...
	const updatePointsTx = await userContract.updatePoints(115);
